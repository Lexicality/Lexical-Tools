...
function PANEL:OnConVarChange(min, val)
	...
	if (val < min) then
		self:DoText(string.format("This server's minimum %s is %d", self:GetCtrlName(), min));
	else
		self:DoText("");
	end
end
...
derma.DefineControl("MinimumValueLabel", "Tell players about defined minima", PANEL, "NagLabel");
